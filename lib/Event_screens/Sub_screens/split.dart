...
class _SplitScreenState extends State<SplitScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (isFirst) {
      provider.splitFetch().then((value) {
        setState(() {
          isLoading = false;
        });
      });
    ...
    return isLoading
        ...
        : provider.splitlist.isEmpty
            ...
            : ListView.builder(
                ...
                itemCount: provider.splitlist.length,
                itemBuilder: (BuildContext context, int index) {
                  return data(
                      exDate: provider.splitlist[index][0] ?? "NA",
                      ratio: provider.splitlist[index][1] ?? "NA",
                      recordDate: provider.splitlist[index][2] ?? "NA",
                      light: light);
                },
              );
  }
